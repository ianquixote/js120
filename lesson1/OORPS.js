...
const RPSGame = {
  ...
  displayWelcomeMessage() {
    ...
    console.log(`Win ${MAX_PTS} rounds to win the game.`);
  },
  ...
  updateAndDisplayScore() {
    ...
    if (playerWins(humanMove, computerMove)) {
      this.score.human += 1;
  ...
  playAgain() {
    ...
    let isValid = ['y', 'yes', 'yep', 'yeah', 'yay',
      'n', 'no', 'nope', 'nah', 'nay'].includes(answer.toLowerCase());
    while (!isValid) {
      console.log('Sorry, that is invalid. Please enter yes or no.');
      answer = readline.question();
      isValid = ['y', 'yes', 'yep', 'yeah', 'yay',
        'n', 'no', 'nope', 'nah', 'nay'].includes(answer.toLowerCase());
    }

    return ['y', 'yes', 'yep', 'yeah', 'yay'].includes(answer.toLowerCase());
  },
  ...
  play() {
    ...
    while (true) {
      ...
      while (this.score.human < MAX_PTS && this.score.computer < MAX_PTS) {
        this.human.choose();
        this.computer.choose(this.human.movePercentage);
        console.clear();
        this.updateAndDisplayScore();
        this.displayMoves('human');
        this.displayMoves('computer');
        this.updateHumanMovePercentage();
        // console.log(this.human.movePercentage);
      }
    ...
    choose() {
      ...
      while (!choice) {
        ...
        Object.keys(VALID_INPUT).forEach((key) => {
          if (VALID_INPUT[key].includes(input.toLowerCase())) {
            choice = key;
          }
        });
...
function playerWins(choice, computerChoice) {
  return LOSERS[choice].includes(computerChoice);
}
